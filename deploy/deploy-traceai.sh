...
if [[ -d "$APP_DIR/.git" ]]; then
  ...
  if [[ -f "$APP_DIR/deploy/git-use-branch.sh" ]]; then
    ...
    git -C "$APP_DIR" -c protocol.version=1 config remote.origin.fetch "+refs/heads/*:refs/remotes/origin/*"
    git -C "$APP_DIR" -c protocol.version=1 fetch --prune origin "refs/heads/${BRANCH}:refs/remotes/origin/${BRANCH}"
    git -C "$APP_DIR" -c protocol.version=1 checkout -B "$BRANCH" "origin/${BRANCH}"
    git -C "$APP_DIR" -c protocol.version=1 pull --ff-only origin "$BRANCH"
  fi
  ...
  rm -rf "$APP_DIR"
  git -c protocol.version=1 clone --branch "$BRANCH" "$REPO_URL" "$APP_DIR"
fi
