...
#
# GitHub may answer anonymous git protocol v2 with HTTP 401 (WWW-Authenticate).
# Git then prompts for a username; over SSH that is "could not read Username".
# Protocol v1 still fetches this public repo without credentials. Do not add
# a token, gh auth, or SSH deploy key for fetch.
set -Eeuo pipefail
...
git() {
  command git -c protocol.version=1 "$@"
}
